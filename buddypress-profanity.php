 ...
 * Description:       This BuddyPress plugin filters out any kind of bad language and ensures your community remains family-friendly, keeping profanity out of posts and comments.
 * Version:           1.9.7
...
if (! defined('BPPROF_PLUGIN_VERSION')) {
	define('BPPROF_PLUGIN_VERSION', '1.9.7');
}
...
function bpprof_same_blog()
{
	echo '<div class="error"><p>'
		. esc_html(__('BuddyPress Profanity requires to be activated on the blog where BuddyPress is activated.', 'buddypress-profanity'))
		. '</p></div>';
}
...
{
	echo '<div class="error"><p>'
		. esc_html(__('BuddyPress Profanity and BuddyPress need to share the same network configuration.', 'buddypress-profanity'))
		. '</p></div>';
}
...
function deactivate_buddypress_profanity()
{
	if (function_exists('is_multisite') && is_multisite()) {
		// Multisite-specific deactivation actions can be added here if needed
	}
}
...
/**
 * Plugin notice while activating on multisite.
 */
...
/**
 * Function to add plugin action links.
 *
...
{
	$wbbprof_links = array(
		'<a href="' . esc_url(admin_url('admin.php?page=buddypress_profanity')) . '">' . esc_html__('Settings', 'buddypress-profanity') . '</a>',
		'<a href="' . esc_url('https://wbcomdesigns.com/contact/') . '" target="_blank">' . esc_html__('Support', 'buddypress-profanity') . '</a>',
	);
	return array_merge($links, $wbbprof_links);
}

/**
 * Function to update blog options.
 *
...
/**
 * Function to initialize plugin if BuddyPress is active.
 */
...
{
	$plugins = filter_input(INPUT_GET, 'plugin');
	if (! isset($plugins)) {
		return;
	}
